...
const api = {
  ...
  getFees: async (filters = {}) => {
    ...
    if (filters.batch && filters.batch !== 'All Batches') {
      enriched = enriched.filter(f => f.student && f.student.batch === filters.batch);
    }
  ...
  downloadReceipt: async (feeId, paymentId = '') => {
    ...
    modal.style.cssText = `background: ${isDark ? '#1e293b' : '#fff'}; border-radius: 16px; width: 100%; max-width: 750px; position: relative; transform: scale(0.95) translateY(20px); transition: all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1); display: flex; flex-direction: column; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); height: 85vh; max-height: 800px;`;
    
    // Header
    const header = document.createElement('div');
    header.style.cssText = `display: flex; justify-content: space-between; align-items: center; padding: 16px 24px; border-bottom: 1px solid ${isDark ? '#334155' : '#e2e8f0'}; background: ${isDark ? '#0f172a' : '#f8fafc'}; border-radius: 16px 16px 0 0; flex-shrink: 0;`;
    header.innerHTML = `
      <h3 style="margin: 0; font-family: 'Outfit', sans-serif; font-weight: 700; font-size: 1.1rem; color: ${isDark ? '#f8fafc' : '#0f172a'};">Receipt Preview</h3>
      <div style="display: flex; gap: 8px;">
        <button id="modal-whatsapp-btn" style="background: #25D366; color: #fff; border: none; padding: 8px 16px; border-radius: 8px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 6px; font-size: 0.85rem; transition: background 0.2s;" onmouseover="this.style.background='#128C7E'" onmouseout="this.style.background='#25D366'"><i class="fa-brands fa-whatsapp"></i> Share</button>
        ...
        <button id="modal-close-btn" style="background: transparent; color: ${isDark ? '#cbd5e1' : '#64748b'}; border: none; padding: 8px; border-radius: 8px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 1.1rem; transition: color 0.2s;" onmouseover="this.style.color='${isDark ? '#fff' : '#0f172a'}'" onmouseout="this.style.color='${isDark ? '#cbd5e1' : '#64748b'}'"><i class="fa-solid fa-xmark"></i></button>
      </div>
    `;
    ...
    const whatsappBtn = header.querySelector('#modal-whatsapp-btn');
    ...
    if (whatsappBtn) {
      whatsappBtn.onclick = async () => {
        if (window.shareInvoiceWhatsApp) {
          window.shareInvoiceWhatsApp(feeId);
        } else {
          try {
            const fees = await api.getFees();
            const fee = fees.find(f => f._id === feeId);
            if (fee && fee.student) {
              const textMessage = encodeURIComponent(`Dear Parent, please find the receipt for ${fee.student.name} for ${fee.billingPeriod}. Thank you.`);
              const waUrl = fee.student.whatsappNumber ? `https://api.whatsapp.com/send?phone=${fee.student.whatsappNumber.replace(/\D/g, '')}&text=${textMessage}` : `https://api.whatsapp.com/send?text=${textMessage}`;
